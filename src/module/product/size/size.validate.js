...
const sizeSchema = z.object({
  short_name: z.string()
    ...
    .regex(/^[a-zA-Z]+$/, { message: "Short name should contain only letters" }),

  full_name: z.string()
    .min(3, { message: "Full name must be at least 3 characters long" })
    .max(50, { message: "Full name cannot exceed 50 characters" })
    .regex(/^[a-zA-Z\s]+$/, { message: "Full name should contain only letters and spaces" }),

  is_active: z.boolean().default(false),
});

const sizeStatusSchema = z.object({
  is_active: z.boolean({
